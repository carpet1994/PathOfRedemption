    ...
    public sealed class GameLoop
    {
        ...
        private void RunLoop(CancellationToken token)
        {
            ...
            while (!token.IsCancellationRequested && _isRunning)
            {
                ...
                {
                    // ── CORREZIONE BUG 1 ──────────────────────────────────────
                    // InputSystem.Update() DEVE essere chiamato all'inizio di ogni
                    // frame, prima di qualsiasi logica di gioco.
                    // Copia _current in _previous: senza questa chiamata,
                    // IsJustPressed() e IsJustReleased() restituiscono sempre
                    // false (o true in loop), rendendo inutilizzabile qualsiasi
                    // input one-shot (conferme, skip cutscene, selezione carte).
